...
`AUTH_MODE=hs256` (a `JWT_SECRET` is auto-generated by the secrets-init job),
CORS open, gateway on `:8000`, with the FastMCP surface mounted at `/mcp`, plus a
ready-to-use `agent@demo.com` account seeded so you can mint a token immediately.
...
- Auth is always on, so the entry needs
  `"headers": { "Authorization": "Bearer <token>" }`. Mint a token via
  `POST /auth/token`, or — easiest path — in the Admin Studio **Users** tab click
  **Generate token** on a user to mint a bearer carrying that user's
  tenant/roles/scopes. The modal hands you a ready-to-paste `mcp.json` (token
  baked in) and a `curl` to verify. Give a demo account the **Demo (can invoke
  tools)** role so its token clears the `/rpc` + `/mcp` gate (the seeded
  `agent@demo.com` already has it).
