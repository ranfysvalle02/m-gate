...
> **TL;DR**
>
> - **A tool is a microservice for agents.** Every hard problem in agent tooling — granularity, discoverability, versioning, security — is one the API and microservice eras already solved. Borrow the answers instead of reinventing them.
> - **Every property of a tool gets multiplied by throughput.** Agents are parallel schedulers, not careful clickers. A bloated response, a vague description, or a non-idempotent write isn't a one-time cost — it scales with call volume, and call volume is compounding.
> - **The description is the API.** The agent picks your tool from a retrieval surface, fresh on every turn. Write the name for lexical match and the description for intent — and stop dumping the whole binder up front.
> - **Make the implementation disposable so the contract can be absolute.** The agent will rewrite the code; the contract, honest side effects, idempotency, and audit trail are what must endure — and you treat that agent as an untrusted client at every boundary.
...
None of that excuses you from the gate at merge time — *test in production* is the second half of a sentence whose first half is *test the contract in CI*. The same contract that makes the implementation disposable is the thing you assert against before it ever ships. Schema-validate every declared input and output, so a drifting shape fails the build instead of a caller. Assert that a tool's declared `action_type` matches what it actually does — a `read` that writes should never go green. And pin the nondeterminism down: mock the agent and the downstream so the test exercises *your* logic and nothing else, replay recorded tool calls as fixtures, and run any model in the loop at temperature zero so a unit test means the same thing twice. CI proves the contract is *satisfiable*; the audit trail proves it *stays* satisfied. You want both gates, precisely because the implementation between them is the part you've agreed to throw away.
