...
Before the mechanics, the premise that makes them necessary: **the model is an untrusted client.** It's fluent and obedient, which makes it *feel* like a trusted piece of your backend — but it reads the open world (web pages, pull requests, emails, search results), so any string it hands you may have been planted by an attacker through prompt injection. Its tool arguments are user input, full stop. That reframes authorization entirely: it can't be something the agent *asserts*, nor something you politely ask it to respect in a system prompt — a prompt is a suggestion, and the agent is gullible at machine speed. Whatever boundary you want has to be enforced *around* the agent, not *by* it. (The companion field guide calls this "the new SQL injection"; the fix is the same one databases settled in 1998 — never trust the caller to govern itself.)
