window.adminConsole = function adminConsole(config) {
  return {
    ...
    state: {
      ...
      // "Draft with AI" (BYOA) modal: generates a sandbox-aware prompt the author
      // pastes into their own AI, then pastes the JSON answer back to auto-fill
      // the function. `tool` is a live reference to the tool being drafted.
      aiAssist: { open: false, tab: "prompt", goal: "", paste: "", tool: null },
    ...
    // ---- "Draft with AI" (BYOA prompt studio) ----------------------------- //
    // We don't call an LLM ourselves: we hand the author a tightly-engineered,
    // tenant-aware prompt to run in their own AI (Cursor/ChatGPT/Claude), then
    // let them paste the answer back. The existing save-path lint + pip chips
    // remain the real gate, so this stays a *drafting* aid, never auto-publish.
    openAiAssist(tool) {
      this.state.aiAssist.tool = tool;
      this.state.aiAssist.tab = "prompt";
      if (!this.state.aiAssist.goal && tool?.description) {
        this.state.aiAssist.goal = tool.description;
      }
      this.state.aiAssist.open = true;
    },

    closeAiAssist() {
      this.state.aiAssist.open = false;
    },

    // Compact "on/off" + list helpers so the generated prompt reads cleanly and
    // never injects a stray empty bullet.
    _aiCap(on) {
      return on ? "on" : "off";
    },

    _aiList(values, emptyText) {
      const items = (values || []).map((v) => String(v).trim()).filter(Boolean);
      return items.length ? items.join(", ") : emptyText;
    },

    // The centerpiece: a dynamically-built prompt that (1) explains the sandbox
    // `context` mental model, (2) injects this tenant's *live* pip / egress /
    // bridge limits so the AI can't suggest something the save-path would
    // reject, and (3) pins best-practice tool-design rules + a strict JSON
    // output contract. Reuses the same policy getters the chips/contract use.
    aiPrompt() {
      const tool = this.state.aiAssist.tool || {};
      const caps = this.sandboxCaps();
      const code = this.codePolicy();
      const http = this.httpPolicy();
      const goal =
        String(this.state.aiAssist.goal || "").trim() ||
        "(describe what this tool should do)";
      const name = String(tool.name || "").trim() || "snake_case_verb_noun";
      const actionType = tool.action_type || "read";

      const packages = code.execution_enabled
        ? this._aiList(
            code.effective,
            "none — standard library only (do not add any third-party package)",
          )
        : "none — code execution is disabled for this tenant";

      const httpReachable =
        caps.http_bridge_enabled && http.enabled
          ? this._aiList(
              http.effective,
              "none — context.http is unavailable; do not make outbound calls",
            )
          : "none — outbound HTTP is OFF for this tenant; do not use context.http";

      return [
        'You are a senior Python engineer writing ONE MCP "code tool" for',
        "mdb-mcp-gateway's network-isolated WebAssembly sandbox. Obey the runtime",
        "contract exactly.",
        "",
        "## Goal",
        goal,
        "",
        "## Runtime: the `context` object  (no sockets, no files, no credentials)",
        "Your function runs in a wasm jail with NO network sockets, NO filesystem,",
        "and NO credentials. Everything external is relayed through the trusted host",
        "via a single injected `context` object — there is no `secrets` arg and no",
        "globals beyond `context`. Reach the outside world ONLY through these four,",
        "each of which REPLACES a standard library you must NOT use:",
        "- context.db    instead of pymongo/MongoClient   -> context.db.users.find_one({...}); ids: context.db.ObjectId(\"..\")",
        "- context.env   instead of os.environ / os.getenv -> context.env.get(\"KEY\", default)",
        "- context.tools instead of re-implementing a sibling tool -> context.tools.<server>.<tool>(**kwargs)",
        '- context.http  instead of requests/httpx/urllib  -> context.http.get(url, params=, auth="ENV_KEY")  (pass the secret NAME, never its value)',
        "FORBIDDEN (the sandbox lint will reject these): importing os/sys/socket/",
        "subprocess/requests/httpx/urllib for I/O, reading os.environ, opening files,",
        "or using raw sockets. For timestamps use the stdlib:",
        "`from datetime import datetime, timezone`. Builtins & exceptions need no import.",
        "",
        "## This tenant's LIVE limits (do not exceed)",
        `- Sandbox bridges: DB ${this._aiCap(caps.db_bridge_enabled)}, cross-tool ${this._aiCap(
          caps.tool_bridge_enabled,
        )}, outbound HTTP ${this._aiCap(caps.http_bridge_enabled)}.`,
        `- Installable pip packages (tenant policy ∩ platform ceiling): ${packages}.`,
        `- Reachable HTTP hosts (deny-by-default): ${httpReachable}.`,
        "",
        "## Design rules (these are graded)",
        `- One tool, one job. Name the function exactly the tool name: ${name}.`,
        "- Declare typed, named parameters (no single `payload` blob). Return a SMALL,",
        "  JSON-serializable dict — distill results, cap arrays, drop nulls; never dump",
        "  a raw multi-thousand-token upstream response.",
        '- Treat every argument as untrusted input: validate types/ranges and raise',
        '  ValueError("clear message") on bad input. Never interpolate arguments into a',
        "  query/command — pass them as data.",
        `- Pick the NARROWEST correct action_type (host-enforced — a read tool literally`,
        "  cannot write): read = no mutations | write = inserts/updates | destructive =",
        `  deletes/irreversible. Current selection is "${actionType}"; change it in your`,
        "  answer if the logic needs more.",
        "- Make writes safe to call twice (upsert / dedupe on a natural key) — agents",
        "  retry and parallelize.",
        "- Fail in the contract: raise clear exceptions; do not return raw stack traces",
        "  or silent empty results.",
        "",
        "## Output — return EXACTLY this JSON object and nothing else",
        "{",
        '  "raw_code": "def NAME(...) -> dict: ...",        // the full Python source',
        '  "requirements": [],                               // [] if stdlib-only; only packages from the allowed list above',
        '  "action_type": "read",                            // read | write | destructive',
        '  "input_schema": {"type":"object","properties":{},"required":[]},',
        '  "description": "one line of intent, in the user\'s words, for tool search"',
        "}",
        "",
        "### Example of the contract (good vs. bad)",
        "BAD (rejected by the sandbox):",
        "    import os, requests",
        "    def fx(q):",
        '        return requests.get("https://api.x.com?q=" + q,',
        '                            headers={"k": os.environ["KEY"]}).json()',
        "GOOD (sandbox-native, validated, distilled):",
        "    def search_widgets(query: str, limit: int = 5) -> dict:",
        '        if not query.strip():',
        '            raise ValueError("query is required")',
        '        resp = context.http.get("https://api.x.com/search",',
    ...
    // Parse the AI's pasted answer (tolerant of ```json fences / surrounding
    // prose) and fill the function fields. We never bypass the human: after
    // applying we re-run the same validation the chips/save use, so an AI mistake
    // surfaces immediately and the author still drives Test/Save.
    applyAiDraft() {
      const tool = this.state.aiAssist.tool;
      if (!tool) return;
      const raw = String(this.state.aiAssist.paste || "").trim();
      if (!raw) {
        this.setError("Paste the AI's JSON answer first.");
        return;
      }
      let data;
      try {
        data = JSON.parse(this._extractJsonBlock(raw));
      } catch (error) {
        this.setError(
          `Couldn't parse the AI answer as JSON: ${error.message || error}. ` +
            "Paste just the JSON object it returned.",
        );
        return;
      }
      if (!data || typeof data !== "object" || Array.isArray(data)) {
        this.setError("The pasted answer must be a single JSON object.");
        return;
      }

      if (typeof data.raw_code === "string" && data.raw_code.trim()) {
        // Routes through the editor-sync helper so the CodeMirror view updates
        // (not just the bound model) and a validate is scheduled.
        this._setToolSource(tool, data.raw_code);
      }
      ...
      if (["read", "write", "destructive"].includes(data.action_type)) {
        tool.action_type = data.action_type;
      }
      if (data.input_schema && typeof data.input_schema === "object") {
        tool.input_schema = JSON.stringify(data.input_schema, null, 2);
      }
      if (typeof data.description === "string" && data.description.trim()) {
        tool.description = data.description.trim();
      }
      // Only adopt a suggested name when the author hasn't set one, so we never
      // silently rename an existing function out from under them.
      if (
        typeof data.name === "string" &&
        data.name.trim() &&
        !String(tool.name || "").trim()
      ) {
        tool.name = data.name.trim();
      }

      this.scheduleToolValidate(tool);
      this.state.aiAssist.open = false;
      this.state.aiAssist.paste = "";
      this.notify(
        "AI draft applied. Review it, then Test and Save — the package chips and " +
          "linter still apply.",
        "success",
      );
    },

    // Pull the first balanced {...} object out of an AI reply, tolerating
    // ```json fences and chatter on either side. Falls back to the raw string.
